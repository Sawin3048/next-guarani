...
let palabraSinAcentos = palabraModificada.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  let cadena = palabraSinAcentos.normalize("NFD").replace(/[\u0300-\u036f']+/g, "").replace('’','')
...
function useRecordAudio() {
  ...
  async function handleAudio(audio: Blob) {
    ...
    const selected = options.find((option,i) => normalizeWord(text).includes(normalizeOptions[i]))
